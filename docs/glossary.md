...
Cobra
    ...
    The various components of Cobra can be found in the [Cobra repository](https://github.com/is4s/pntOS-Python):

    ```{table} Cobra Component Breakdown
    | Component name                                        | Location within the repository        | Module import location | Description                                                                                                                                                                                                                                   |
    |:----------------------------------------------------- |:------------------------------------- | ---------------------- |:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | {ref}`Cobra API <cobra_api>`                          | `pntos-api/src/pntos/api/`            | `pntos.api`            | An {term}`API` written in Python that defines a Position, Navigation, and Timing Operating System ({term}`pntOS`). Consists of [abstract plugin definitions](https://github.com/is4s/pntOS-Python/tree/main/pntos-api/src/pntos/api/plugins). |
    ...
    | [Cobra Extras](./extras.md)                           | `pntos-extras/src/pntos/extras/`      | `pntos.cobra.extras`   | Auxiliary Cobra plugins and tools that are not included with the core components.                                                                                                                                                             |
